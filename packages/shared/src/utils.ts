...
function applyMoveAction(
  gameState: Game.GameNormalState,
  action: MoveAction
): Game.GameNormalState {
...
function applySacrificeAction(
  ...
  const isHomeSystem =
    Game.getHomeSystem(action.player, nextState) === updatedSystem;
  if (isHomeSystem) {
...
function applySetupAction(
  gameState: Game.GameSetupState,
  action: GameAction.GameSetupAction
): Game.GameState {
