...
export function setSystem(
  ...
  if (gameState.tag !== 'normal') throw new Error('Invalid game state');
...
function applyMoveAction(
  ...
  // no longer valid
  const nextState = Game.updateSystem(updatedFrom, gameState);
  ...
  if (action.toSystemId) {
    ...
    return Game.updateSystem(updatedTo, nextState);
  }

  // Otherwise, create the new system (removing the star piece from the bank)
  // and add the ship to it
  if (!action.size || !action.color) throw new Error('New star piece required');
  const [newSystem, newState] = Game.createSystem(
    nextState,
    action.size,
    action.color,
    [ship]
  );
  if (!newSystem) throw new Error('New star piece not found in bank');
  return newState;
}
...
function applyGrowAction(
  ...
  return {
    ...Game.updateSystem(updatedSystem, gameState),
    bank,
  };
}
...
function applyTradeAction(
  ...
  return {
    ...Game.updateSystem(nextSystem, gameState),
    bank: updatedBank,
  };
}
...
function applySacrificeAction(
  ...
  const cleanedState = Game.updateSystem(updatedSystem, gameState);
...
function applyOverpopulationAction(
  ...
  const cleanedUp = Game.updateSystem(updatedSystem, gameState);
